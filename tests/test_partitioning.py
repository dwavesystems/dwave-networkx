...
class TestPartitioning(unittest.TestCase):
    ...
    #     node_partitions = dnx.partition(G, sampler=LeapHybridDQMSampler())
    #     self.assertTrue(node_partitions == {})

    #     node_partitions = dnx.weighted_partition(G, sampler=LeapHybridDQMSampler())
    #     self.assertTrue(node_partitions == {})
    ...
    def test_typical_cases(self):
        ...
        node_partitions = dnx.partition(G, num_partitions=5, sampler=LeapHybridDQMSampler())
        for i in range(5):
        ...
        with self.assertRaises(dnx.DWaveNetworkXException):
            node_partitions = dnx.weighted_maximum_cut(G, sampler=LeapHybridDQMSampler())

        nx.set_edge_attributes(G, 1, 'weight')
        node_partitions = dnx.weighted_maximum_cut(G, sampler=LeapHybridDQMSampler())
        self.assertTrue(sum(x == 0 for x in node_partitions.values()) == 5)  # half of the nodes in subset '0'

        node_partitions = dnx.weighted_partition(G, num_partitions=5, sampler=LeapHybridDQMSampler())
        for i in range(5):
            self.assertTrue(sum(x == i for x in node_partitions.values()) == 2) # 5 equally sized subsets
