...
def pegasus_layout(G, scale=1., center=None, dim=2, crosses=False):
    ...
    G : NetworkX graph
        ...
        the :func:`dwave.graphs.pegasus_graph` function.
    ...
    >>> pos = dwave.graphs.pegasus_layout(G)

    """

    if not isinstance(G, nx.Graph) or G.graph.get("family") != "pegasus":
        raise ValueError("G must be generated by dwave.graphs.pegasus_graph")
...
def draw_pegasus(G, crosses=False, **kwargs):
    ...
    >>> G = dwave.graphs.pegasus_graph(2)
    >>> dwave.graphs.draw_pegasus(G)    # doctest: +SKIP
